...
- Its like trying different paths, and when you hit a dead end, you backtrack to the last choice
and try a different route. 
- It is commonly used in situations where you need to explore multiple possibilities to solve
a problem, like searching for a path in a maze or solving puzzles like Sudoku. When the dead end
is reached, the algorithm backtracks to the previous decision point and explores a different path 
...
+ Partial Solution: An intermediate or incomplete configuration
...
+ Decision Point:Aspecific step in the algorithm where a candidate is chosen and added to the 
partial solution.
+ Feasible Solution: A partial or complete solution that adheres
to all contraints.
...
+ Does not always need backtracking.          + Uses recursion to solve problem
+ Breaking them into smaller, similar         + Solving problems with multiple choices and exploring
subproblems and solving recursively             options systematically, backtracking when needed
+ Controlled by function calls and            + Managed explicitly with loops and state
call stack
+ Ex: Tree, Graph Traversal, Merge Sort,      + Ex: Sudoku, Maze, Graph color, Knight Tour
Quick Sort, Binary Search

How does it work?
- By recursively exploring all possible solutions to a problem. It starts by choosing an intial 
position, and then it explores all possible extensions of that solution. If an extension leads
to a solution, the algorithm returns that solution. If an extension does not lead to a solution,
the algorithm backtracks to the prev solution and tries a different extension
1. Choose an initial solution
2. Explore all possible extensions of the current solution
3. If an extension leads to a solution, return that solution
4. If an extension does not lead to a solution, backtrack to the previous solution and try a different
extension.
...
4. If moving in that direction does not lead to the ending point, backtrack to the prev position
